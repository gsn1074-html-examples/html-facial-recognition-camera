//Reference: https://github.com/auduno/clmtrackr
...
function draw() {
    ...
    var positions = ctracker.getCurrentPosition();  // get array of face marker positions [x, y] format

    if (saving) {
        
        image(videoInput, 0, 0, width, height); //draw video image behind the prop on the canvas just for this frame
    }

    if (positions.length > 1) {
        
        var ang = atan2(positions[27][1] - positions[32][1], positions[27][0] - positions[32][0]);  //angle between eyes gives angle of head
        
        var d = int(dist(positions[27][0], positions[27][1], positions[32][0], positions[32][1]));  //distance between eyes so can work out scale
        
        var scl = map(d, 0, 290, 0, 1.8);

        if (mode == 1) {    //33 position between eyes -- glasses
        ...
        } else if (mode == 2) { //37 position base of nose -- moustache
        ...
        } else if (mode == 3) { //41 position mid nose -- whiskers
        ...
        } else if (mode == 4) { //56 position left mouth -- pipe
        ...
        } else if (mode == 5) { //33 to 37 length of nose place this distance above 33 for ears  -- ears
        ...
        } else if (mode == 6) { //33 to 37 length of nose place this distance above 33 for ears -- hat
        ...
        } else if (mode == 7) { //20 position left eyebrow
